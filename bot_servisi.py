...
NOT: Bu dosya artık trading_engine.py'a bağımlı DEĞİL (tek dosya deploy
sorununu ortadan kaldırmak için tüm ortak mantık buraya gömüldü). Aynı
mantığın bir kopyası streamlit_app.py içinde de var; ikisini birlikte
...
def sanal_cuzdan_kaydet(cuzdan):
    return _json_kaydet(SANAL_CUZDAN_DOSYASI, cuzdan)
...
def calistir_backtest(df, baslangic_sermaye=10000):
    ...
    return {
        "son_bakiye": bakiye, "toplam_islem": toplam_islem,
        "win_rate": win_rate, "net_kar_yuzde": net_kar_yuzde, "islemler": islemler,
    }
...
def _render_bekleme(toplam_saniye, bekleme_dk):
    """Render uyku moduna geçmesin diye 60 sn adımlarla bekler ve heartbeat loglar."""
    kalan = 0
